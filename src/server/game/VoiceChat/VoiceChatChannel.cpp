...
void VoiceChatChannel::SendAvailableVoiceChatChannel(WorldSession* session)
{
    ...
    TC_LOG_DEBUG("voicechat", "Sending SMSG_AVAILABLE_VOICE_CHANNEL for plr %u", plr->GetGUID());
...
void VoiceChatChannel::SendVoiceRosterUpdate(bool empty, bool toAll, ObjectGuid toPlayer)
{
    ...
    data << uint32(htonl(sVoiceChatMgr->GetVoiceServerAddress()));
    ...
    for (const auto & m_member : m_members)
    {
        ...
        Player* plr = ObjectAccessor::FindConnectedPlayer(m_member.second.m_guid);
        if (!plr)
        {
            m_is_deleting = true;
        ...
        for (const auto & j : m_members)
        {
            ...
            // clean up if disconnected
            Player* other_plr = ObjectAccessor::FindConnectedPlayer(j.second.m_guid);
            if (!other_plr)
            {
                m_is_deleting = true;
...
void VoiceChatChannel::SendLeaveVoiceChatSession(WorldSession* session) const
{
    ...
    TC_LOG_DEBUG("voicechat", "Sending SMSG_VOICE_SESSION_LEAVE for plr %u", plr->GetGUID());
...
void VoiceChatChannel::SendLeaveVoiceChatSession(ObjectGuid guid)
{
    ...
    {
        if (Player* plr = ObjectAccessor::FindConnectedPlayer(memb.second.m_guid))
        {
...
void VoiceChatChannel::AddVoiceChatMember(ObjectGuid guid)
{
    ...
    if (!plr)
    {
        TC_LOG_ERROR("voicechat", "could not add voice member, player not found!");
        return;
    }
    ...
    if (!sess)
    {
        TC_LOG_ERROR("voicechat", "could not add voice member, sess not found!");
        return;
    }

    if (!sess->IsVoiceChatEnabled())
    {
        TC_LOG_ERROR("voicechat", "could not add voice member, voice chat disabled!");
        return;
    }
    ...
    if (!user_id)
    {
        TC_LOG_ERROR("voicechat", "could not add voice member, no free slots!");
...
void VoiceChatChannel::RemoveVoiceChatMember(ObjectGuid guid)
{
    ...
    if (!member)
    {
        TC_LOG_ERROR("voicechat", "error in remove voice member, member not found!");
        return;
    }
    ...
    TC_LOG_DEBUG("voicechat", "user #%u removed from channel #%u", member->user_id, m_channel_id);
...
void VoiceChatChannel::AddMembersAfterCreate()
{
    ...
    TC_LOG_DEBUG("voicechat", "VoiceChat: Adding voice chat enabled members after create, channel #%u, type #%u", m_channel_id, m_type);
    ...
    switch (GetType())
    {
        ...
        case VOICECHAT_CHANNEL_BG:
        {
            if (Battleground* bg = sBattlegroundMgr->GetBattleground(GetGroupId(), BATTLEGROUND_TYPE_NONE))
            {
                ...
                {
                    if (Player* bgMember = ObjectAccessor::FindConnectedPlayer(itr.first))
                    {
        ...
        case VOICECHAT_CHANNEL_CUSTOM:
        {
            ...
            {
                Channel* chan = cMgr->GetChannel(GetChannelName());
                if (chan)
                {
...
void VoiceChatChannel::VoiceMember(ObjectGuid guid)
{
    ...
    if (!member)
    {
        TC_LOG_ERROR("voicechat", "error in voice voice member, member not found!");
        return;
    }
    ...
    TC_LOG_DEBUG("voicechat", "user #%u voiced in channel #%u", member->user_id, m_channel_id);
}
...
void VoiceChatChannel::DevoiceMember(ObjectGuid guid)
{
    ...
    if (!member)
    {
        TC_LOG_ERROR("voicechat", "error in devoice voice member, member not found!");
        return;
    }
    ...
    TC_LOG_DEBUG("voicechat", "user #%u devoiced in channel #%u", member->user_id, m_channel_id);
}
...
void VoiceChatChannel::MuteMember(ObjectGuid guid)
{
    ...
    if (!member)
    {
        TC_LOG_ERROR("voicechat", "error in mute voice member, member not found!");
        return;
    }
    ...
    TC_LOG_DEBUG("voicechat", "user #%u muted in channel #%u", member->user_id, m_channel_id);
}
...
void VoiceChatChannel::UnmuteMember(ObjectGuid guid)
{
    ...
    if (!member)
    {
        TC_LOG_ERROR("voicechat", "error in unmute voice member, member not found!");
        return;
    }
    ...
    TC_LOG_DEBUG("voicechat", "user #%u unmuted in channel #%u", member->user_id, m_channel_id);
}
...
void VoiceChatChannel::ForceMuteMember(ObjectGuid guid)
{
    ...
    if (!member)
    {
        TC_LOG_ERROR("voicechat", "error in force mute voice member, member not found!");
        return;
    }
    ...
    TC_LOG_DEBUG("voicechat", "user #%u force muted in channel #%u", member->user_id, m_channel_id);
}
...
void VoiceChatChannel::ForceUnmuteMember(ObjectGuid guid)
{
    ...
    if (!member)
    {
        TC_LOG_ERROR("voicechat", "error in force unmute voice member, member not found!");
        return;
    }
    ...
    TC_LOG_DEBUG("voicechat", "user #%u force unmuted in channel #%u", member->user_id, m_channel_id);
}
