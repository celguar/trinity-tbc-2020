...
public:
    ...
    // configs
    uint32 GetVoiceServerAddress() const { return server_address; }
    uint16 GetVoiceServerConnectPort() const { return server_port; }
    uint16 GetVoiceServerVoicePort() const { return voice_port; }
    std::string GetVoiceServerAddressString() { return server_address_string; }

    // manage voice channels
    void CreateVoiceChatChannel(VoiceChatChannelTypes type, uint32 groupId = 0, const std::string& name = "", Team team = TEAM_OTHER);
    void DeleteVoiceChatChannel(VoiceChatChannel* channel);
    bool IsVoiceChatChannelBeingCreated(VoiceChatChannelTypes type, uint32 groupId = 0, const std::string& name = "", Team team = TEAM_OTHER);
...
private:
    ...
    std::shared_ptr<Trinity::Asio::IoContext> m_ioContext = std::make_shared<Trinity::Asio::IoContext>();
